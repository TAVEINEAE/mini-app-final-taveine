...
function renderCart() {
    ...
    footer.innerHTML = `
        ...
        <button class="add-btn checkout-btn">Checkout</button>
    `;
    ...
    if (wishlist.length === 0) {
        container.innerHTML = `
            <div class="empty-wish">
                ...
                <button class="add-btn" onclick="closePage('wish-page')">
                    Browse collection
                </button>
            </div>`;
    } else {
    ...
    document.getElementById('search-results-products').innerHTML = results.length
        ? results.map(p => `
            <div class="search-result-item" onclick="closeSearch();openProductDetail('${p.id}')">
                <img src="${p.image||'https://via.placeholder.com/60x75'}" alt="${p.name}">
                <div>
